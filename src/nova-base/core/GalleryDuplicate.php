...
class GalleryDuplicate extends Gallery
{
    ...
    private function filesCanBeDeleted($file0, $file1): bool
    {
        ...
        if (!Album::fileExists($file0)) {
            echo "file0 not exists $file0<br>\n";
            return false;
        }
        if (!Album::fileExists($file1)) {
            echo "file1 not exists $file1<br>\n";
            return false;
        }

        $md50 = FileSystem::md5(Album::getFullFilename($file0));
        $md51 = FileSystem::md5(Album::getFullFilename($file1));
        return $md50 === $md51;
    }
